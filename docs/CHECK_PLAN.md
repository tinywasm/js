...
| # | Tarea | Done |
|---|---|---|
| 0 | **Precondición externa:** publicar `tinywasm/fetch` con `func NewResponse(status int, headers []Header, body []byte) *Response` | [x] |
| 1 | Definir tipos `Request` (`Headers []fetch.Header`), `Message` (`Data []byte`). La respuesta de OnFetch es `*fetch.Response` | [x] |
| 2 | Definir interfaces `ServiceWorkerHandler`, `WebWorkerHandler` | [x] |
| 3 | Implementar registro global de handlers con detección de duplicados | [x] |
| 4 | Implementar funciones puente `__tinywasm_sw_*` y `__tinywasm_worker_*` (build `wasm`) | [x] |
| 5a | Añadir `require` de `github.com/tinywasm/fmt`, `tinywasm/context`, `tinywasm/fetch`, `tinywasm/json` al `js/go.mod` | [x] |
| 5b | Migrar embeds `wasm_exec_go.js` / `wasm_exec_tinygo.js` desde `client/assets/` a `js/assets/` con getters **privados** `wasmExecGo()` / `wasmExecTinyGo()` | [x] |
| 5c | Exponer mínima superficie pública: `Runtime`/`RuntimeGo`/`RuntimeTinyGo`, `SetRuntime`. Resto privado. | [x] |
| 6a | Constructor `PageBootstrap()` que compone `wasm_exec` + `WebAssembly.instantiateStreaming(fetch("/client.wasm"))` y devuelve `*Script{Name:""}` | [x] |
| 6b | Constructores `ServiceWorker(handler)` y `WebWorker(name, handler)` que componen sus shims usando el runtime activo y devuelven `*Script` con `Content` final | [x] |
| 7 | Detección de contexto (page/SW/worker) en el bootstrap del runtime | [x] |
| 8 | Suite de tests en `tests/` — `gotest ./...` verde (skill [[testing]]) | [x] |
| 9 | Actualizar `README.md` + crear `docs/ARCHITECTURE.md` | [x] |
